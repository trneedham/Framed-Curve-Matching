function [dist,geod,pushoff_geod]=OpenGeod_RMF(p1,p2)
 
%input: two curves p1 and p2. Entered as samples along the curves, as 3xn1
%       and 3xn2 matrices. It is not required for n1=n2.
%output: dist=geodesic distance between framed curves with rotation minimizing framing. 
%             Curves have been optimally registered over rotations and
%             reparameterizations.
...
% Form the quaternionic curve representation
    q1 = curve2quat_RMF(p1);
    q2 = curve2quat_RMF(p2);
...
% Send back to a curve
    [p2,~,~]=quat_to_framed_curve(q2);

% Applying optimal re-parameterization to the second curve
    [gam] = DynamicProgrammingQ2(q1,q2,0,0);
    gamI = invertGamma(gam);
    gamI = (gamI-gamI(1))/(gamI(end)-gamI(1));
    p2new = Group_Action_by_Gamma_Coord(p2,gamI);
    q2new = curve2quat_RMF(p2new);
...
% Create figures
    ...
    if Disp_geodesic_tubed_curves
        figure(104); clf;
        pics_to_show = [1,20,40,60,80,100];
