function [dist,geod,pushoff_geod]=OpenGeod_RMF_Opt(p1,p2)
...
%             its RMF framing and the second has a framing which has 
...
% Create figures
    ...
    if Disp_geodesic_tubed_curves
        ...
        for j=1:length(pics_to_show)
            tubeplot(geod(:,:,pics_to_show(j)),frames_geod(:,:,pics_to_show(j)),thickness,5);
            axis equal
            set(gca,'xtick',[])
            set(gca,'ytick',[])
            set(gca,'ztick',[])
