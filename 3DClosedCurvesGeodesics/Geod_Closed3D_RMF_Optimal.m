function [dist,geod,frames_geod,pushoff_geod,gamI]=Geod_Closed3D_RMF_Optimalv2(p1,p2)
...
% Choose some parameters. These can be adjusted by the user.
    ...
    k=25;   % Number of optimal seeds to take for the initial parameterizations.
...
% cause problems with the assumption that the curves are immersed.
    if p1(:,N)==p1(:,1)
        p1(:,N)=7/8*p1(:,N)+1/8*p1(:,N-1);
    end
    
    if p2(:,N)==p2(:,1)
        p2(:,N)=7/8*p2(:,N)+1/8*p2(:,N-1);
    end
...
% then find an optimal framing for V2 w.r.t. this orientation.
    [p1,p2,~,V2]=Optimal_Frame_RMF(p1,p2);
    
% Find the best k seeds for p2, with this initial parameterization.
    seeds=best_seeds_framing_RMF(p1,p2,V2,k);

% Find the optimal reparameterization from the given seeds.
    [z1best,w1best,z2best,w2best,~] = Optimal_Reparam_Framing_RMF(p1,p2,V2,seeds);
