function [z1best,w1best,z2best,w2best,gamIbest] = Optimal_Reparam_ArbFrame(p1,V1,p2,V2,seeds)

% Input: A pair of closed space curves p1, p2, a framing V2 for p2, and some seed positions for p2.
...
for j=1:k
    ...
    if distj < minDist
        z1best=z1new;
        w1best=w1new;
        z2best=z2new*norm(z1best)/norm(z2new);
        w2best=w2new*norm(w1best)/norm(w2new);
%         z1best=z1new/norm(z1new);
%         w1best=w1new/norm(w1new);
%         z2best=z2new/norm(z2new);
%         w2best=w2new/norm(w2new);
        gamIbest=gamI;
        minDist=distj;
    end
end
